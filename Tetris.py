...
#player_name = raw_input('Type in name you would like to appear on High Score Charts: ')
...
txt_lines = "LINES: "
txt_lvl = "LEVEL: "
txt_hscore = "HIGH SCORES"

high_scores = [line.split(', ') for line in file('Highscores.txt', 'r').read().split('\n')[1:]]
while len(high_scores) > 3:
	drop = high_scores.pop()
...
txt_hscore_center = (center_x, center_y + 120)

text_hscores = []
for high_score in high_scores[:3]:
	text_hscores.append(font_sl.render(high_score[0] + ": " + high_score[1], True, (255, 255, 255)))
txt_hscores_pos = []
for txt in text_hscores:
	txt_hscores_pos.append(txt.get_rect())
for txt in xrange(len(txt_hscores_pos)):
	txt_hscores_pos[txt].center = (center_x, center_y + 180 + 60*txt)
...
while run_game:
	...
	text5 = font_sl.render(txt_hscore, True, (255, 255, 255))
	...
	txt_hscore_pos.center = txt_hscore_center
	...
	screen.blit(text5, txt_hscore_pos)
	for txt in xrange(len(high_scores)):
		screen.blit(text_hscores[txt], txt_hscores_pos[txt])
...
for hs in xrange(len(high_scores)):
	if score > high_scores[hs][1]:
		high_scores.insert(hs, [player_name, str(score), str(lines), str(speed - 1)])
new_high_scores = file("Highscores.txt", 'w')
nhs = ''
for hs in high_scores:
	...
	nline = nline.strip(', ')
	nhs += nline + '\n'
new_high_scores.write('Name, Score, Lines, Level\n' + nhs)
new_high_scores.close()
